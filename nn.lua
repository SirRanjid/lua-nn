...
local function pt(tbl,d,n)
	local otabs = string.rep("\t",d)
	print((n and "["..n.."] = ("..tostring(tbl)..")\n" or "")..otabs.."{")
	d = d+1
...
local func = {
	...
	arlu = function(x) return math.max(0,1) end, --#
...
setmetatable(nn,{__call = function(self,x,y,yi,yo,act,drv,lr)
	...
	for I = 1,x do
		...
		if I == 1 then --input layer
			for J = 1,yi do
				new.net[I][J] = {0,0,0,0,0,act = act, drv = drv}
			end
...
function nn:backprop(tbl,j)
	...
	local err_v, err_t = self:getError(tbl)
...
function nn:train(inp,exp_out,j)
	--print("exp_out0",tostring(exp_out))
	if not j then
		--print("noexp")
	...
	if self:run(inp) then
		--print("exp_out",tostring(exp_out))
...
function nn:smart_train(train_pairs,p)
	...
	if pc[1] < pc[2] then
		...
		local err,err_t = self:train(pair,pc[3]*self.lr^2)
		self:pairupdate(pair,err)
	end
end
...
function nn:getPairCache(pair)
	--print("nn:gpc",self.paircache[pair],pair)
